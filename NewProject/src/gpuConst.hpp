...
__global__ void CUDA_convolution_const_laplacian(short *image, short *result,
  int Nx, int Ny);
...
/* This function implements the shared memory and constant kernels convolution in CUDA for Laplacian */
    ...
    for (int y = -kerRad; y <= kerRad; ++y){
      for (int x = -kerRad; x <= kerRad; ++x){
          ...
          tmp +=
            (gaussianKernel[x+kerRad + M*(y+kerRad)] * data[idx+x][idy+y]);
        }
    }
...
/* This function implements the shared memory and constant kernels convolution in CUDA for Laplacian */
    ...
    for (int y = -kerRad; y <= kerRad; ++y){
      for (int x = -kerRad; x <= kerRad; ++x){
          if ((i+x < 0) || (j+y < 0) || (i+x >= Nx) || (j+y >= Ny)) continue;
          tmp +=
            (laplacianKernel[x+kerRad + M*(y+kerRad)] * data[idx+x][idy+y]);
        }
    }
...
/* This function implements the call to the CUDA shared memory and constant kernels convolution */
  ...
  switch (filterChoice) {
    case gaussian:
      cudaMemcpyToSymbol(gaussianKernel, &gaussFilt[0], sizeFilt);
      CUDA_convolution_const_gaussian<<<blocks,threads>>>(d_img, d_res, Nx, Ny);
      break;
    case laplacian:
      cudaMemcpyToSymbol(laplacianKernel, &LaplacianFilt[0], sizeFilt);
      CUDA_convolution_const_laplacian<<<blocks,threads>>>(d_img, d_res, Nx, Ny);
      break;
   }
